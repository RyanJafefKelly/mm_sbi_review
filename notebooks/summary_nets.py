# Note: quick fix for pickle object
...
class RickerSummary(nn.Module):
    def __init__(self, input_size, hidden_dim):
        ...
        self.conv = nn.Sequential(
            nn.Conv1d(self.input_size, self.hidden_dim, 3, 3),
...
class GLRSummary(nn.Module):
    def __init__(self, input_dim, hidden_dim):
        ...
        self.rho = nn.Sequential(
            nn.Linear(hidden_dim, hidden_dim), nn.ReLU(), nn.Linear(hidden_dim, 20)
        )
...
class OUPSummary(nn.Module):
    def __init__(self, input_size, hidden_dim, N):
        ...
        self.conv = nn.Sequential(
            nn.Conv1d(self.input_size, 8, 3, 2),
            nn.Conv1d(8, 8, 3, 2),
            nn.Conv1d(8, 2, 3, 2),
            nn.AvgPool1d(2),
        )
    ...
    def forward(self, Y):
        ...
        out, (embeddings_lstm, c) = self.lstm(
            Y.reshape(self.N * batch_size, 25, 1), (hidden, c)
        )
